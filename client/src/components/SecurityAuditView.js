 ...
 * Every finding shows file:line reference and the actual code snippet that triggered it.
 */
export function renderSecurityAuditView(container, securityData = {}) {
  const warnings = securityData.security_warnings || [];
  const codeSmells = securityData.code_smells_and_technical_debt || [];
      ...
      <div class="security-list">
        ${warnings.length > 0 ? warnings.map(w => {
            ...
            <div class="security-card ${borderClass}">
              <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;">
                <div style="flex: 1;">
                  ...
                  <h4 style="font-size: 1.05rem; font-weight: 700; color: var(--text-primary); margin-top: 0.2rem;">${w.issue}</h4>
          ...
          <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); padding: 1.5rem; border-radius: var(--radius-md); text-align: center;">
            ...
            <h4 style="color: #34d399; font-weight: 700;">No Security Findings</h4>
            <p style="font-size: 0.85rem; color: var(--text-secondary); margin-top: 0.25rem;">No hardcoded secrets, injection vectors, or insecure patterns detected in scanned files.</p>
          </div>
        `}
      </div>
    </div>
        ...
        <table class="custom-table">
          ...
          <tbody>
            ${codeSmells.length > 0 ? codeSmells.map(smell => `
              <tr>
                <td><span class="badge" style="background: rgba(245, 158, 11, 0.15); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.3);">${smell.category}</span></td>
                <td>${smell.description}</td>
                <td style="color: var(--text-secondary);">${smell.impact}</td>
              </tr>
            `).join('') : `<tr><td colspan="3" style="text-align: center; color: var(--text-muted);">No significant technical debt detected</td></tr>`}
          </tbody>
        </table>
      </div>
    </div>
